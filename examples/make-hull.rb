...
gunnelf = curve(ptsf, vcsf)
lines << gunnelf

# サイドデッドフラットライン
sdfpa = [pp_aft - (pp_aft - ae) / 2.0, b2, d]
sdfpf = [pp_fore + (xtop_max - pp_fore) / 3.0, b2, d]
sdfla = curve [sdfpa, [pp_aft - (pp_aft-ae) / 3.0, b2, d / 2.0], [pp_aft, b2, br]], [[0,0,-1],nil,[1,0,0]]
sdflf = curve [[pp_fore, b2, br], sdfpf], [[1,0,0],[0,0,1]]
lines << sdfla << sdflf

# ボトムデッドフラットライン
blae = [-ae + -ae / 5.0, 0, 0]
blfe = [lpp - (loa-lpp)/2.0, 0, 0]
bdfla = curve [blae, [pp_aft, b2 - br, 0]], [[1,0,0],[1,0,0]]
bdflf = curve [[pp_fore, b2 - br, 0], blfe], [[1,0,0],[1,-0.5,0]]
lines << bdfla << bdflf

# トランサム
tsom_tl = line([ae, 0, d], [ae, b4, d])
tsom_bl = curve [[ae, 0, d * 2.0 / 3.0], [ae, b4, d]], [[0,1,0], [0,0,1]]
...
tp3 = curve [[-ae, 0, d / 2.0], [-ae + -ae / 5.0, 0, d / 4.0]], [v1.to_a, v2.to_a]
...
p4 = [lpp, 0, d / 4.0 * 3.0]
p5 = [xtop_max, 0, d]
fore_profile = curve [p1, p2, [xtop_max,0,d/2.0], p3, p4, p5], [[1,0,0], [0,0,1], nil, [0,0,1], [0,0,1], (p5.to_v - p4.to_v).to_a]
lines << fore_profile
